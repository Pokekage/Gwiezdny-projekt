...
// --- 6. HOROSKOP ---
function showHoroscope(sign, signIndex) {
    const textIndex = getDailyIndex(horoscopeTexts, signIndex);
    const recipeIndex = getDailyRecipeIndex(signIndex); // każdy znak ma swój przepis!
...
function renderRecipes() {
    ...
    const dailyIndex = getDailyIndex(allRecipes, 0);
    const daily = allRecipes[dailyIndex];
    const now = new Date();
    ...
    dailyCard.innerHTML = `
        <span class="daily-badge">🌟 POLECANE NA DZIŚ — ${now.toLocaleDateString('pl-PL')} 🌟</span>
        <h3>${daily.t}</h3>
        <p>${daily.d}</p>
        <span style="font-size:0.75rem; color: rgba(212,175,55,0.6); margin-top:8px; display:block;">Kliknij, by zobaczyć przepis</span>
    `;
    dailyCard.onclick = () => openModal(daily.t, daily.m);
...
function clearDreams() {
    if (confirm('Usunąć wszystkie zapisane sny?')) {
